...
class BenchmarkCNN(object):
  ...
  def _benchmark_cnn(self):
    ...
    with sv.managed_session(
      ...
      global_step_watcher = GlobalStepWatcher(
          sess, global_step,
          self.num_warmup_batches + init_global_step,
      ...
      log_fn('total images/sec: %.2f' %
             (global_step_watcher.steps_per_second() * self.batch_size * MPI_SIZE))
      log_fn('-' * 64)
  ...
  def _build_model(self):
    ...
    with tf.device(self.cpu_device):
      nclass, images_splits, labels_splits = add_image_preprocessing(
          ...
          1, input_data_type, self.resize_method,
          not FLAGS.eval)

    staging_delta_ops = []

    results = self.add_forward_pass(
        images_splits[0], labels_splits[0], nclass,
        phase_train, input_data_type, data_type, input_nchan,
    ...
    with tf.device(self.param_server_device):
      ...
      if gradient_clip is not None:
        clipped_grads = [
            ...
            for grad, var in device_grads
        ]
      else:
        clipped_grads = device_grads

      training_ops.append(opt.apply_gradients(clipped_grads))
    train_op = tf.group(*(training_ops + update_ops))

    with tf.device(self.cpu_device):
      if MPI_RANK == 0 and FLAGS.summary_verbosity > 0:
  ...
  def add_forward_pass(
      self, host_images, host_labels, nclass, phase_train,
